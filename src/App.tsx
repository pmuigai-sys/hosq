...
import { LogOut, Users, Shield, Activity, Monitor } from 'lucide-react';

function AppContent() {
  const { user, userRole, signOut, isAdmin, isTriage, loading } = useAuth();
  const [view, setView] = useState<'patient' | 'staff' | 'display'>('patient');
  ...
  if (user && userRole) {
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        {isAdmin ? <AdminDashboard /> : isTriage ? <TriageDashboard /> : <EmployeeDashboard />}
      </div>
    );
  }
